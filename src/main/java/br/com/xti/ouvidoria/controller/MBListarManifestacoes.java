...
public class MBListarManifestacoes implements Serializable {
    ...
    private void ajustarCaixaEntrada(List<TbManifestacao> list) {
        for (TbManifestacao tbManifestacao : list) {
            Collection<TbEncaminhamento> encaminhamentos = this.encaminhamentoDAO
                    .getEncaminhamentoAberto(tbManifestacao);
            for (TbEncaminhamento tbEncaminhamento : encaminhamentos) {
                tbEncaminhamento.setTbTramiteCollection(this.tramiteDAO.getPorEncaminhamento(tbEncaminhamento));
            }
            tbManifestacao.setTbEncaminhamentoCollection(encaminhamentos);
        }
        switch (securityService.getUserProfile()) {
            case INTERLOCUTOR: // remover as manifestações que ele já encaminhou
                               // para um operador.
                ...
                for (; listIterator.hasNext();) {
                    enc: for (TbEncaminhamento e : listIterator.next().getTbEncaminhamentoCollection()) {
                        if (e.getIdUnidadeEnviou().equals(unidadeInterlocutor)) {
                            ...
                            // Verifica o último trâmite para determinar se deve
                            // estar ou não na caixa de entrada
                            ...
                            if (!UnidadeEnum.OUVIDORIA.getId().equals(
                                    ultimoTramite.getIdUsuarioEmissor().getIdUnidade().getIdUnidade())) {
                            ...
                            for (TbTramite t : listTramites) {
                                if (t.getIdUsuarioReceptor() != null
                                        && FuncaoUsuarioEnum.OPERADOR.getId().equals(
                                                t.getIdUsuarioReceptor().getTpFuncao())
                ...
                for (; listIterator.hasNext();) {
                    enc: for (TbEncaminhamento e : listIterator.next().getTbEncaminhamentoCollection()) {
                        if (e.getIdUnidadeRecebeu().equals(unidadeInterlocutor)) {
                            ...
                            ListIterator<TbTramite> listTramites = new ArrayList<TbTramite>(e.getTbTramiteCollection())
                                    .listIterator(e.getTbTramiteCollection().size());
                            // Percorre a lista em ordem inversa para pegar o
                            // último trâmite da Ouvidoria
                            for (; listTramites.hasPrevious();) {
                                TbTramite t = listTramites.previous();
                                if (t.getIdUsuarioEmissor() != null
                                        && UnidadeEnum.OUVIDORIA.getId().equals(
                                                t.getIdUsuarioEmissor().getIdUnidade().getIdUnidade())) {
                            ...
                            for (TbTramite t : e.getTbTramiteCollection()) {
                                if (!(t.getIdUsuarioReceptor() != null && FuncaoUsuarioEnum.OPERADOR.getId().equals(
                                        t.getIdUsuarioReceptor().getTpFuncao()))) {
                                    continue;
                                }

                                if (BooleanEnum.NAO.getId().equals(t.getStRetornada())
                                        && t.getDtTramite().compareTo(dtUltimoTramiteDaOuvidoria) >= 0) {
                                    // if(BooleanEnum.NAO.getId().equals(t.getStRetornada()))
                                    // { //TODO
                                    listIterator.remove();
                                    break enc;
    ...
    private void ajustarDevolvidas(List<TbManifestacao> list) {
        switch (securityService.getUserProfile()) {
            case OPERADOR: // remover as manifestações que foram retornadas mas
                           // voltou para o operador
    ...
    public String getUnidadesEncaminhadas(TbManifestacao manifestacao) {
        ...
        // Collection<TbEncaminhamento> listaEncaminhamentos =
        // manifestacao.getTbEncaminhamentoCollection();
        Collection<TbEncaminhamento> listaEncaminhamentos = this.encaminhamentoDAO
                .getEncaminhamentoAberto(manifestacao);
    ...
    public int diasAtrasoAoManifestante(TbManifestacao manifestation) {
        ...
        if (manifestation != null
            ...
            if (securityService.isInterlocutor() || securityService.isOperador()) {
                ...
                int diasParaResponderManifestante = prazoEncaminhamento + prazoRespostaAoManifestante
                        + prazoRespostaAOuvidoria;
        ...
        if (securityService.isOuvidor() || securityService.isAdministrador()) {
            ...
            if (!StatusManifestacaoEnum.SOLUCIONADA.getId().equals(manifestacao.getStStatusManifestacao())) {
                ...
                Collection<TbEncaminhamento> listaEncaminhamentos = this.encaminhamentoDAO
                        .getEncaminhamentoAberto(manifestacao);
                if (listaEncaminhamentos != null) {
                    if (prazoEncaminhamento > 0 && diasTranscorridos > prazoEncaminhamento
                ...
                if (todasAreasRetornaram) {
                    ...
                    // no session or session was closed
                    // Collection<TbComunicacaoExterna> listaComunicacao =
                    // manifestacao.getTbComunicacaoExternaCollection();
                    Collection<TbComunicacaoExterna> listaComunicacao = this.comunicacaoExternaDAO
                    ...
                    if (ValidacaoHelper.isEmpty(listaComunicacao)) {
                        ...
                        // pega a ultima comunicacao e verifica se ela possui
                        // resposta final
