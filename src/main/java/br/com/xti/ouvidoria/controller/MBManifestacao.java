...
public class MBManifestacao extends AbstractManifestationController implements Serializable {
    ...
    private static final String MENSAGEM_ATRASO_UNIDADE = "Esta manifestação se encontra em atraso por parte %s %s. Caso queira notificar os envolvidos favor clicar no botão ao lado";
    ...
    // "Orão de Destino", "Classificação", "Subclassificação" e
    // "Área Solucionadora"
    ...
    public void loadManifestation() {
        if (this.manifestacao == null) {
            ...
            // atualiza variavel de controle caso a manifestação já possua uma
            // mensagem do OUVIDOR ao MANIFESTANTE
            mensagemAoManifestante = false;
            if (this.manifestacao != null) {
                // failed to lazily initialize a collection of role:
                // br.com.xti.ouvidoria.model.TbManifestacao.tbComunicacaoExternaCollection,
                // no session or session was closed
                // Collection<TbComunicacaoExterna> communicationList =
                // manifestacao.getTbComunicacaoExternaCollection();
                Collection<TbComunicacaoExterna> communicationList = this.comunicacaoExternaDAO
                        .getPorManifestacao(manifestacao);
                    ...
                    for (TbComunicacaoExterna externalCommunication : communicationList) {
                        ...
                        if (userCommunication != null) {
                            ...
                            if (userProfileId != null) {
                                ...
                                if (userProfile == FuncaoUsuarioEnum.OUVIDOR
                                        || userProfile == FuncaoUsuarioEnum.ADMINISTRADOR) {
    ...
    private void init() {
        // Mantem os dados do gráfico no Escopo Flash, caso tenha vindo da
        // página de gráfico de questionário
        ...
        // TbManifestacao.tbClassificacaoCollection, no session or session was
        // closed
        // if(ValidacaoHelper.isNotEmpty(manifestacao.getTbClassificacaoCollection()))
        // {
        manifestacao.setTbClassificacaoCollection(this.classificacaoDAO.getClassificacoesPorManifestacao(manifestacao
                .getIdManifestacao()));
        if (ValidacaoHelper.isNotEmpty(manifestacao.getTbClassificacaoCollection())) {
        ...
        // failed to lazily initialize a collection of role:
        // br.com.xti.ouvidoria.model.TbManifestacao.tbSubClassificacaoCollection,
        // no session or session was closed
        // if(ValidacaoHelper.isNotEmpty(manifestacao.getTbSubClassificacaoCollection()))
        // {
        manifestacao.setTbSubClassificacaoCollection(this.subClassificacaoDAO
                .getSubClassificacoesPorManifestacao(manifestacao.getIdManifestacao()));
        if (ValidacaoHelper.isNotEmpty(manifestacao.getTbSubClassificacaoCollection())) {
        ...
        //br.com.xti.ouvidoria.model.TbManifestacao.tbUnidadeAreaSolucionadoraCollection, no session or session was closed
        //List<TbUnidade> listAreaSolucionadora = new ArrayList<>(manifestacao.getTbUnidadeAreaSolucionadoraCollection());
        List<TbUnidade> listAreaSolucionadora = new ArrayList<>(this.unidadeDAO.getPorEncaminhamentoManifestacao(manifestacao));
        ...
        //failed to lazily initialize a collection of role: br.com.xti.ouvidoria.model.TbManifestacao.tbEncaminhamentoCollection, no session or session was closed
        //Collection<TbEncaminhamento> encaminhamentos = manifestacao.getTbEncaminhamentoCollection();
        manifestacao.setTbEncaminhamentoCollection(this.encaminhamentoDAO.getEncaminhamentoAberto(manifestacao));
        Collection<TbEncaminhamento> encaminhamentos = manifestacao.getTbEncaminhamentoCollection();
        for (TbEncaminhamento tbEncaminhamento : encaminhamentos) {
            tbEncaminhamento.setIdManifestacao(this.manifestacaoDAO.getManifestacaoPorNumero(manifestacao.getNrManifestacao()));
        }
        ...
        //br.com.xti.ouvidoria.model.TbManifestacao.tbManifestacaoxAnexoCollection, no session or session was closed
        manifestacao.setTbManifestacaoxAnexoCollection(this.manifestacaoDAO.getManifestacaoAnexoPorManifestacao(manifestacao.getIdManifestacao()));
        //br.com.xti.ouvidoria.model.TbComunicacaoExterna.tbComunicacaoExternaxAnexoCollection, no session or session was closed
    ...
    public boolean showTakeUpManifestation() {
        return (isManifestacaoNova() || hasUsuarioAnalisador()) && (!isManifestacaoEmAnalise())
    ...
    public boolean showSecretData() {
        ...
        if (securityService.isManifestante()
                || securityService.hasRole(FuncionalidadeEnum.VISUALIZAR_DADOS_SIGILOSOS.toString())) {
            showSecretData = true;
        } else {
    ...
    public boolean mostrarBotaoResponderAOuvidoria() {
        ...
        if (!StatusManifestacaoEnum.SOLUCIONADA.getId().equals(manifestacao.getStStatusManifestacao())
                && securityService.isInterlocutor()) {
            TbEncaminhamento e = ManifestacaoUtils.getEncaminhamentoDoUsuarioLogado(manifestacao);
            //br.com.xti.ouvidoria.model.TbEncaminhamento.tbTramiteCollection, no session or session was closed
            //List<TbTramite> listTramites = new ArrayList<TbTramite>(e.getTbTramiteCollection());
            List<TbTramite> listTramites = new ArrayList<TbTramite>(this.tramiteDAO.getPorEncaminhamento(e));
            ...
            if (e.getIdUnidadeEnviou().getIdUnidade() == UnidadeEnum.OUVIDORIA.getId()) {
                if (!(UnidadeEnum.OUVIDORIA.getId().equals(ultimoTramite.getIdUnidadeEnvio().getIdUnidade()) || ultimoTramite
                        .getIdUnidadeEnvio() == null)) {
                    mostrar = true;
                }
            }
        }
    ...
    public boolean desativarBotaoResponderQuestionario() {
        if (StatusManifestacaoEnum.SOLUCIONADA.getId().equals(manifestacao.getStStatusManifestacao())) {
            if (manifestacao.getIdQuestionario() == null
                    || ValidacaoHelper.isEmpty(manifestacao.getIdQuestionario().getIdQuestionario())) {
    ...
    public void handleFileUpload(FileUploadEvent event) throws IOException {
        ...
        if (uploadedFile != null) {
            try {
                ...
                String pathName = FileHelper
                        .getUploadedFilePathname(uploadedFilename, manifestacao.getNrManifestacao());
                ...
                try {
                    ...
                    MensagemFaceUtil.info("Arquivo enviado",
                            String.format("Arquivo enviado com sucesso: %s", file.getName()));
    ...
    public void removerArquivo(TbAnexo anexo, Collection<TbAnexo> listaArquivos) {
        if (ValidacaoHelper.isNotEmpty(listaArquivos)) {
            ...
            try {
                ...
                if (file.exists()) {
                    if (file.delete()) {
                        MensagemFaceUtil.info("Arquivo excluido",
                                String.format("Arquivo excluido com sucesso: %s", file.getName()));
    ...
    public TbEncaminhamentoPadronizado getEncaminhamentoPadronizadoTramite() {
        return encaminhamentoPadronizadoDAO.find(idEncaminhamentoPadronizadoTramite == null ? 0
                : idEncaminhamentoPadronizadoTramite);
    }
    ...
    public void alteraTipoManifestacao() {
        try {
            TbTipoManifestacao tipomanifestacao = tipomanifestacaoDAO.find(manifestacao.getIdTipoManifestacao()
                    .getIdTipoManifestacao());
            ...
            dao.edit(manifestacao);
            MensagemFaceUtil.info(
                    "Alteração de Tipo de Manifestação",
                    String.format("Tipo de manifestação alterado de %s para %s.", tipoAnterior,
                            tipomanifestacao.getNmTipoManifestacao()));
        } catch (InfrastructureException ex) {
    ...
    public List<String> getIdClassificacao() {
        // if(ValidacaoHelper.isNotEmpty(manifestacao.getTbClassificacaoCollection()))
        // {
        // for (TbClassificacao classificacao :
        // manifestacao.getTbClassificacaoCollection()) {
        // try {
    ...
    public void ajustaClassificacao() {
        // Se selecionar a Ouvidoria em orgão de destino aparece todas as
        // classificações
    ...
    public void ajustaSubClassificacoes() {
        try {
            ...
            for (TbClassificacao tbClassificacao : listClassificacao) {
            ...
            MensagemFaceUtil.info("Gravação de Classificação",
                    "Ocorreu um erro ao gravar Classificação: " + ex.getMessage());
        }
        Collections.sort(subClassificacoes);
        subClassificacoesCompleta = new ArrayList<TbSubClassificacao>(subClassificacoes);
    }
    ...
    public void gravaSubClassificacao() {
        try {
            ...
            MensagemFaceUtil.erro("Gravação de Subclassificação",
                    "Ocorreu um erro ao gravar subclassificação: " + ex.getMessage());
        }
    }
    ...
    public void gravaPrioridade() {
        try {
            ...
            ex.printStackTrace();
            MensagemFaceUtil
                    .erro("Alteração de Prioridade", "Ocorreu um erro ao gravar prioridade: " + ex.getMessage());
        }
    }
    ...
    public void gravaStatusManifestacao() {
        try {
            ...
            if (StatusManifestacaoEnum.SOLUCIONADA.getId().equals(manifestacao.getStStatusManifestacao())) {
                ...
                // retorna manifestacao ao status anterior ate o usuario
                // confirmar a acao de marcar como solucionada
            ...
            ex.printStackTrace();
            MensagemFaceUtil.erro("Alteração de Status",
                    "Ocorreu um erro ao gravar o status da manifestação: " + ex.getMessage());
        }
    }
        ...
        for (Integer id : tramitesEscolhidos) {
            ...
            // Anexos do trâmite
            {
                ...
                for (TbTramitexAnexo anexoTramite : anexos) {
                    try {
                        File file = new File(parametroDAO.getDiretorioAnexo()
                                + String.format("%d_%d_%s", manifestacao.getNrManifestacao(), new Date().getTime(),
                                        anexoTramite.getIdAnexo().getNmAnexo()));
                        TbAnexo anexo = new TbAnexo();
                        anexo.setNmAnexo(anexoTramite.getIdAnexo().getNmAnexo());
                        anexo.setDsCaminhoAnexo(file.getName());
                        FileUtils.copyFile(new File(parametroDAO.getDiretorioAnexo()
                                + anexoTramite.getIdAnexo().getDsCaminhoAnexo()), file); // faz
                                                                                         // uma
                                                                                         // cópia
                                                                                         // do
                                                                                         // arquivo
    ...
    public void gravaSolucionarManifestacao() {
        ...
        if (respostaManifestacao != null) {
            ...
            String textoPadraoSemPalavrasChave = PalavrasChavesHelper.converterPalavrasChaves(
                    textoPadraoComPalavrasChave, manifestacao, false);
    ...
    public void gravaSolucionarManifestacaoREAL() {
        if (ValidacaoHelper.isNotEmpty(idUnidadeAreaSolucionadora)) {
            ...
            MensagemFaceUtil.erro("Para solucionar uma manifestação deve ser selecionada uma unidade solucionadora",
                    "Unidade Solucionadora");
            return;
        }
        ...
        try {
            ...
            if ((manifestacao.getEeEmailUsuario() != null || manifestacao.getEeEmailSecundario() != null)
                    && "1".equals(manifestacao.getStResposta())) {

                try {
                    EmailService.Email emailManifestante = emailService.newEmail();

                    // adiciona email primario como destinatario
                    String nomeManifestante = ValidacaoHelper.isNotEmpty(manifestacao.getNmPessoa()) ? manifestacao
                            .getNmPessoa() : "Manifestante";
                    emailManifestante.addDestinatario(nomeManifestante, manifestacao.getEeEmailUsuario());

                    // ----- adicionando anexos ao email ---------//
                    for (TbAnexo anexo : arquivosComunicacaoExterna) {
                        emailManifestante.addAnexo(diretorioAnexo + anexo.getDsCaminhoAnexo(), anexo.getNmAnexo(),
                                anexo.getNmAnexo());
                    }
                    ...
                    TbEmailAutomatizado emailAutomatizado = emailAutomatizadoDAO
                            .findByTipo(EmailAutomatizadoEnum.RESPOSTA_FINAL);
                    ...
                    emailManifestante.setAssunto(PalavrasChavesHelper.converterPalavrasChaves(
                            emailAutomatizado.getNmTituloEmail(), manifestacao, dsMensagemAoManifestante, false));
                    emailTextoHtml.append(PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getDsEmail(),
                            manifestacao, dsMensagemAoManifestante, false));
                    emailTexto.append(PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getDsEmail(),
                            manifestacao, dsMensagemAoManifestante, false));

                    // emailManifestante.setAssunto("[MinC Ouvidoria] Encaminhamento de Manifestação para análise: "
                    // + manifestacao.getNrManifestacao());
                    ...
                    MensagemFaceUtil
                            .alerta("O Status da Manifestação foi gravado com sucesso, porém não foi possível enviar o e-mail ao Manifestante",
                                    null);
            ...
            try {
                ...
                if (questionario != null) {
                    ...
                    TbEmailAutomatizado emailAutomatizado = emailAutomatizadoDAO
                            .findByTipo(EmailAutomatizadoEnum.RESPONDER_QUESTIONARIO);

                    // adiciona texto e assunto de email padrao
                    StringBuilder emailTextoHtml = new StringBuilder();
                    StringBuilder emailTexto = new StringBuilder();

                    emailQuestionario.setAssunto(PalavrasChavesHelper.converterPalavrasChaves(
                            emailAutomatizado.getNmTituloEmail(), manifestacao, dsMensagemAoManifestante, false));
                    emailTextoHtml.append(PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getDsEmail(),
                            manifestacao, dsMensagemAoManifestante, false));
                    emailTexto.append(PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getDsEmail(),
            ...
            e.printStackTrace();
            MensagemFaceUtil.erro("Ocorreu um erro ao tentar redirecionar o usuário",
                    "Ocorreu um erro ao tentar redirecionar o usuário");
        }
    ...
    public void salvaEncaminhamento() {
        ...
        for (String idUnidadeStr : idOrgaoDestino) {
            try {
                ...
                manifestacao.setDtUltimaAtualizacao(new Date());
                // atualiza status da manifestacao caso ela ainda esteja como
                // nova
                if (manifestacao.getStStatusManifestacao().equals(StatusManifestacaoEnum.EM_ANALISE.getId())
                        || manifestacao.getStStatusManifestacao().equals(
                ...
                try {
                    ...
                    if (qtdEncaminhamento == 1) {
                        ...
                        if ((manifestacao.getEeEmailUsuario() != null || manifestacao.getEeEmailSecundario() != null)
                            ...
                            String nomeManifestante = (manifestacao.getNmPessoa() != null) ? manifestacao.getNmPessoa()
                                    : "Manifestante";
                            emailManifestante.addDestinatario(nomeManifestante, manifestacao.getEeEmailUsuario());

                            TbEmailAutomatizado emailAutomatizado = emailAutomatizadoDAO
                                    .findByTipo(EmailAutomatizadoEnum.PRIMEIRO_TRAMITE);
                            ...
                            emailManifestante.setAssunto(PalavrasChavesHelper.converterPalavrasChaves(
                                    emailAutomatizado.getNmTituloEmail(), manifestacao, false));
                            emailTextoHtml.append(PalavrasChavesHelper.converterPalavrasChaves(
                                    emailAutomatizado.getDsEmail(), manifestacao, false));
                            emailTexto.append(PalavrasChavesHelper.converterPalavrasChaves(
                                    emailAutomatizado.getDsEmail(), manifestacao, false));
                    ...
                    MensagemFaceUtil.info("Manifestação encaminhada com sucesso.",
                            "Manifestação encaminhada com sucesso.");
                } catch (Exception e) {
                    e.printStackTrace();
                    MensagemFaceUtil
                            .alerta("Manifestação encaminhada mas não possível notiticar por e-mail.",
                                    "O seu encaminhamento foi realizado com sucesso porém, não foi possível notificar os envolvidos por e-mail.");
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void enviarEmailTramiteOperador(TbUsuario usuarioReceptor) {
        try {
            if (usuarioReceptor != null && StatusUsuarioEnum.ATIVO.getId() == usuarioReceptor.getStStatus()
                ...
                // adiciona texto e assunto de email padrao
                String emailTexto = PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getDsEmail(),
                        manifestacao, true);

                email.setAssunto(PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getNmTituloEmail(),
                        manifestacao, true));
    ...
    private void enviarEmailsEncaminhamento(TbUnidade unidade, TbUsuario usuarioReceptor) {
        ...
        String emailTexto = PalavrasChavesHelper.converterPalavrasChaves(emailAutomatizado.getDsEmail(), manifestacao,
                true);
        ...
        try {
            ...
            if (securityService.isManifestante()) {
                manifestacao.setStStatusManifestacao(StatusManifestacaoEnum.EM_ANDAMENTO.getId());
                    ...
                    || StatusManifestacaoEnum.SOLICITACAO_RESPONDIDA.getId().equals(
                            manifestacao.getStStatusManifestacao())
            ...
            if (securityService.isManifestante()) {
                ...
                // recupera texto padrao para o email
                TbEmailAutomatizado emailAutomatizado = emailAutomatizadoDAO
                        .findByTipo(EmailAutomatizadoEnum.MANIFESTANTE);
    ...
    public void salvaTramite() {
        try {
            ...
            if (ValidacaoHelper.isNotEmpty(idUsuarioTramite)) {
                ...
                TbUnidade unidade = unidadeDAO.find(idUnidadeTramite);
                unidade.setTbUsuarioCollection(this.usuarioDAO.findOuvidorByUnidade(unidade.getIdUnidade()));
                enviarEmailsEncaminhamento(unidade, null);
            }

            tramiteDAO.create(tramite);
            /*------- fim - gravando tramite ------------*/

            // atualiza status do encaminhamento dependendo de para onde a msg
            // foi enviada
            TbUnidade unidadeEnviou = manifestacaoTabView.getUnidadeEnviou();
            TbEncaminhamento encaminhamento = encaminhamentoDAO.find(manifestacaoTabView.getEncaminhamento()
                    .getIdEncaminhamento());
            encaminhamento.setTbTramiteCollection(this.tramiteDAO.getPorEncaminhamento(encaminhamento));
            if (unidadeEnviou.getIdUnidade().equals(idUnidadeTramite)
                    && !(securityService.isOuvidor() || securityService.isAdministrador())) {
                encaminhamento.setStEncaminhamento(StatusEncaminhamentoEnum.RETORNADA.getId());
                encaminhamento.setDtRespostaTramite(new Date());

                // Se retornar a mensagem para a Unidade que realizou o
                // encaminhamento
            ...
            // atualiza status dos tramites TbEncaminhamento.tbTramiteCollection
    ...
    public void ajustaDescricaoMensagemManifestante() {
        mensagemManifestante = PalavrasChavesHelper.converterPalavrasChaves(dsMensagemAoManifestante, manifestacao,
                false);
    }
    ...
    /**
     * Prepara a lista de unidades que podem ser áreas solucionadoras. Apenas as
     * unidades que já receberam encaminhamentos podem ser áreas solucionadoras,
     * além da Ouvidoria que sempre pode ser.
     */
    public void preparaListaAreasSolucionadoras() {
        unidades = new ArrayList<>();
        // Adiciona as unidades que já receberam encaminhamento
        // failed to lazily initialize a collection of role:
        // br.com.xti.ouvidoria.model.TbManifestacao.tbEncaminhamentoCollection,
        // no session or session was closed
        // for (TbEncaminhamento referral :
        // manifestacao.getTbEncaminhamentoCollection()) {
        for (TbEncaminhamento referral : this.encaminhamentoDAO.getEncaminhamentoAberto(manifestacao)) {
    ...
    public Boolean habilitaEnvioMensagemManifestante() {
        if (manifestacao.getIdUsuarioAnalisador() != null) {
            ...
            } else if (manifestacao.getStStatusManifestacao().equals(StatusManifestacaoEnum.EM_ANALISE.getId())
                    && manifestacao.getIdUsuarioAnalisador().getIdUsuario() != securityService.getUser().getIdUsuario()) {
                return true; // desabilita elemento
            } else {
                return false; // nao desabilita elemento
            }
        ...
        if (!securityService.isInterlocutor()) {
            if (ValidacaoHelper.isEmpty(idOrgaoDestino, idClassificacao, idPrioridade)) {
                ...
                if (manifestacao.getIdUsuarioAnalisador() != null) {
                    if (StatusManifestacaoEnum.NOVA.getId().equals(manifestacao.getStStatusManifestacao())) {
                        desabilitar = true;
                    } else if (StatusManifestacaoEnum.EM_ANALISE.getId().equals(manifestacao.getStStatusManifestacao())
                            && !isLoggedUserResponsibleForManifestation()) {
                        desabilitar = true;
                    }
                } else {
                    desabilitar = true;
    ...
    public Collection<TbUsuario> getUsuarios() {
        ...
        if (idUnidade != UnidadeEnum.OUVIDORIA.getId()) {
            for (Iterator<TbUsuario> it = usuariosDaUnidade.iterator(); it.hasNext();) {
                ...
                if ((!usuario.getTpFuncao().equals(FuncaoUsuarioEnum.OPERADOR.getId())) || usuario.isInativo()) {
                    it.remove();
                }
            }
        }
    ...
    public void ajustaListaStatusManifestacao() {
        ...
        // Verifica se e permitido encerrar com status encaminhado na
        // preferencia dos sistema e se existe encaminhamento aberto
        ...
        switch (status) {
            ...
            case EM_ANALISE:
                listaStatusManifestacao.add(StatusManifestacaoEnum.EM_ANALISE);
                if (encerrar) { // disponibiliza status apenas se nao exisitr
                                // encaminhamento aberto
    ...
    public void generateNewPassword() {
        try {
            ...
            MensagemFaceUtil.erro("Não foi possível notificar o manifestante por email sobre a alteração da senha",
                    null);
            e.printStackTrace();
        }
    }
     ...
     *            Comunicação Externa a ser marcada (ou desmarcada) como pública
     * @throws Exception
    ...
    /**
     * Marca o trâmite como público ou privado. fazendo com que as unidades
     * possam ver a mensagem e baixar os anexos
     * 
     * @param tramiteObj
    ...
    public String nomeBotaoTramiteEscolhido(Integer idTramite) {
        ...
        if (ValidacaoHelper.isNotEmpty(idTramite) && tramitesEscolhidos.contains(idTramite)) {
        ...
        for (TbEncaminhamento encaminhamento : encaminhamentosAtrasados) {
            ...
            try {
                ...
                for (TbUsuario usuario : usuarios) {
                    ...
                    if (funcao == FuncaoUsuarioEnum.INTERLOCUTOR
                            && StatusUsuarioEnum.ATIVO.getId() == usuario.getStStatus()) {
                ...
                for (TbTramite tramite : tramites) {
                    ...
                    if (usuario != null && StatusUsuarioEnum.ATIVO.getId() == usuario.getStStatus()) {
                        FuncaoUsuarioEnum funcao = EnumHelper.getFuncaoUsuarioEnum(usuario.getTpFuncao());
                        if (funcao == FuncaoUsuarioEnum.OPERADOR
                                && BooleanEnum.NAO.getId().equals(tramite.getStRetornada())) {
                            enviarEmail(usuario.getNmUsuario(), usuario.getEeEmail(),
                ...
                e.printStackTrace();
                MensagemFaceUtil.alerta("Ocorreu um erro ao tentar notificar os envolvidos por email sobre o atraso",
                        null);
            }
        }
        showRedirectPageModal();
     ...
     * @param emailType
     *            padrão de email a ser utilizado
     * @param isExternalEmail
     *            TRUE se o email for enviado para manifestantes (usuários
     *            externos)
     * @throws EmailException
     */
    private void enviarEmail(String userName, String userEmail, EmailAutomatizadoEnum emailType, boolean isExternalEmail)
            throws EmailException {
        EmailService.Email email = emailService.newEmail();
        TbEmailAutomatizado emailTemplate = emailAutomatizadoDAO.findByTipo(emailType);
        String emailContent = PalavrasChavesHelper.converterPalavrasChaves(emailTemplate.getDsEmail(), manifestacao,
                isExternalEmail);

        email.addDestinatario(userName, userEmail);
        email.setAssunto(PalavrasChavesHelper.converterPalavrasChaves(emailTemplate.getNmTituloEmail(), manifestacao,
                isExternalEmail));
    ...
    private String getTextoTramiteAtraso() {
        return new StringBuilder()
                ...
                .append("<p>Atenciosamente,<br />").append("Ouvidoria<br />").append("Ministério da Cultura</p>")
                .toString();
    }
    ...
    public void ajustaListaUnidadeTramite(ManifestacaoTabView manifestationTabView) {
        ...
        if (securityService.isOperador()) {
            if (departmentSent.getIdUnidade() == UnidadeEnum.OUVIDORIA.getId() && userDepartment.isRetornoOuvidoria()) {
    ...
    public boolean disableManifestationType() {
        return isManifestacaoBloqueada()
                || (!desabilitaEdicaoManifestacao() ? !(securityService.isOuvidor() || securityService
                        .isAdministrador()) : desabilitaEdicaoManifestacao());
    }
