...
public class ManifestacaoDAO extends AbstractDAO<TbManifestacao> {
    ...
    /**
     * Retorna uma lista de manifestações que tenham um encaminhamento feito
     * para uma unidade específica e que não tenha sido transferida para nenhum
     * operador pelo interlocutor responsável pela Unidade. Ou seja, o
     * encaminhamento está parado na unidade porém sem nenhum responsável por
     * ela.
     * 
     * @param idUnidade
     *            Id da Unidade
    ...
    private Predicate[] getFiltros(CriteriaBuilder cb, Root<TbManifestacao> m, FiltroPersonalizado filtro) {
        ...
        Join<TbManifestacao, TbUnidade> joinAreaSolucionadora = m.join(
                TbManifestacao_.tbUnidadeAreaSolucionadoraCollection, JoinType.LEFT);
        Join<TbManifestacao, TbTipoManifestacao> joinTipoManifestacao = m.join(TbManifestacao_.idTipoManifestacao,
                JoinType.LEFT);
        Join<TbManifestacao, TbSubClassificacao> joinSubClassificacao = m.join(
                TbManifestacao_.tbSubClassificacaoCollection, JoinType.LEFT);
        Join<TbManifestacao, TbClassificacao> joinClassificacao = m.join(TbManifestacao_.tbClassificacaoCollection,
                JoinType.LEFT);
        ...
        Join<TbManifestacao, TbGrauInstrucao> joinGrauInstrucao = m
                .join(TbManifestacao_.idGrauInstrucao, JoinType.LEFT);
        Join<TbManifestacao, TbAreaEntrada> joinAreaEntrada = m.join(TbManifestacao_.idAreaEntrada, JoinType.LEFT);

        Join<TbManifestacao, TbEncaminhamento> joinEncaminhamento = m.join(TbManifestacao_.tbEncaminhamentoCollection,
                JoinType.LEFT);
        Join<TbEncaminhamento, TbUnidade> joinUnidadeEnviou = joinEncaminhamento.join(
                TbEncaminhamento_.idUnidadeEnviou, JoinType.LEFT);
        Join<TbEncaminhamento, TbUnidade> joinUnidadeRecebeu = joinEncaminhamento.join(
                TbEncaminhamento_.idUnidadeRecebeu, JoinType.LEFT);
        Join<TbEncaminhamento, TbUsuario> joinUsuarioEnviou = joinEncaminhamento.join(
                TbEncaminhamento_.idUsuarioEnviou, JoinType.LEFT);
        Join<TbEncaminhamento, TbTramite> joinTramite = joinEncaminhamento.join(TbEncaminhamento_.tbTramiteCollection,
                JoinType.LEFT);

        // Id da Manifestação
        if (ValidacaoHelper.isNotEmpty(filtro.getManIdRangeDe())
                && ValidacaoHelper.isNotEmpty(filtro.getManIdRangeAte())) {
            restricoesLista.add(cb.between(m.get(TbManifestacao_.nrManifestacao), filtro.getManIdRangeDe(),
                    filtro.getManIdRangeAte()));
        }
        ...
        if (filtro.getManDataCadastroDe() != null && filtro.getManDataCadastroAte() != null) {
            restricoesLista.add(cb.between(m.get(TbManifestacao_.dtCadastro),
                    DataHelper.getDataMin(filtro.getManDataCadastroDe()),
                    DataHelper.getDataMax(filtro.getManDataCadastroAte())));
        }
        if (filtro.getManDataAtualizacaoDe() != null && filtro.getManDataAtualizacaoAte() != null) {
            restricoesLista.add(cb.between(m.get(TbManifestacao_.dtUltimaAtualizacao),
                    DataHelper.getDataMin(filtro.getManDataAtualizacaoDe()),
                    DataHelper.getDataMax(filtro.getManDataAtualizacaoAte())));
        }
        if (filtro.getManDataFechamentoDe() != null && filtro.getManDataFechamentoAte() != null) {
            restricoesLista.add(cb.between(m.get(TbManifestacao_.dtFechamento),
                    DataHelper.getDataMin(filtro.getManDataFechamentoDe()),
                    DataHelper.getDataMax(filtro.getManDataFechamentoAte())));
        } // Tipo Manifestação
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getManIdGrauInstrucao())) {
            restricoesLista.add(joinGrauInstrucao.get(TbGrauInstrucao_.idGrauInstrucao).in(
                    filtro.getManIdGrauInstrucao()));
        } // Raça
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getManIdClassificacao())) {
            restricoesLista.add(joinClassificacao.get(TbClassificacao_.idClassificacao).in(
                    filtro.getManIdClassificacao()));
        }
        if (ValidacaoHelper.isNotEmpty(filtro.getManClassificacao())) {
            restricoesLista.add(cb.like(cb.upper(joinClassificacao.get(TbClassificacao_.dsClassificacao)), "%"
                    + filtro.getManClassificacao().toUpperCase() + "%"));
        } // Sub-Classificacao
        if (ValidacaoHelper.isNotEmpty(filtro.getManIdSubClassificacao())) {
            restricoesLista.add(joinSubClassificacao.get(TbSubClassificacao_.idSubClassificacao).in(
                    filtro.getManIdSubClassificacao()));
        }
        if (ValidacaoHelper.isNotEmpty(filtro.getManSubClassificacao())) {
            restricoesLista.add(cb.like(cb.upper(joinSubClassificacao.get(TbSubClassificacao_.dsSubClassificacao)), "%"
                    + filtro.getManClassificacao().toUpperCase() + "%"));
        } // Prioridade
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getManPrioridade())) {
            restricoesLista.add(cb.like(cb.upper(joinPrioridade.get(TbPrioridade_.nmPrioridade)), "%"
                    + filtro.getManPrioridade().toUpperCase() + "%"));
        } // Área Solucionadora
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getManAreaSolucionadora())) {
            restricoesLista.add(cb.or(
                    cb.like(cb.upper(joinAreaSolucionadora.get(TbUnidade_.nmUnidade)), "%"
                            + filtro.getManAreaSolucionadora().toUpperCase() + "%"),
                    cb.like(cb.upper(joinAreaSolucionadora.get(TbUnidade_.sgUnidade)), "%"
                            + filtro.getManAreaSolucionadora().toUpperCase() + "%")));
        } // Sigilo (dos dados)
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getManTelefone())) {
            restricoesLista.add(cb.like(cb.upper(m.get(TbManifestacao_.nrTelefone)), "%"
                    + filtro.getManTelefone().toUpperCase() + "%"));
        } // Email
        if (ValidacaoHelper.isNotEmpty(filtro.getManEmail())) {
            restricoesLista.add(cb.or(
                    cb.like(cb.upper(m.get(TbManifestacao_.eeEmailUsuario)), "%" + filtro.getManEmail().toUpperCase()
                            + "%"),
                    cb.like(cb.upper(m.get(TbManifestacao_.eeEmailSecundario)), "%"
                            + filtro.getManEmail().toUpperCase() + "%")));
        } // Endereço
        if (ValidacaoHelper.isNotEmpty(filtro.getManEndereco())) {
            restricoesLista.add(cb.like(cb.upper(m.get(TbManifestacao_.enEndereco)), "%"
                    + filtro.getManEndereco().toUpperCase() + "%"));
        } // TextoManifestação
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getManNumProcesso())) {
            restricoesLista.add(cb.like(cb.upper(m.get(TbManifestacao_.nrProcesso)), "%"
                    + filtro.getManNumProcesso().toUpperCase() + "%"));
        } // TipoManifestante
        ...
        // TextoEncaminhamento e Trâmite
        if (ValidacaoHelper.isNotEmpty(filtro.getEncDesc())) {
            restricoesLista.add(cb.like(cb.upper(joinEncaminhamento.get(TbEncaminhamento_.dsDescricao)), "%"
                    + filtro.getEncDesc().toUpperCase() + "%"));
            restricoesLista.add(cb.like(cb.upper(joinTramite.get(TbTramite_.dsDescricao)), "%"
                    + filtro.getEncDesc().toUpperCase() + "%"));
        } // Status doEncaminhamento
        if (ValidacaoHelper.isNotEmpty(filtro.getEncStatus()) && !filtro.getEncStatus().equals("0")) {
            restricoesLista.add(cb.equal(joinEncaminhamento.get(TbEncaminhamento_.stEncaminhamento),
                    filtro.getEncStatus()));
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getEncUnidade())) {
            restricoesLista.add(cb.or(
                    cb.like(cb.upper(joinUnidadeRecebeu.get(TbUnidade_.nmUnidade)), "%"
                            + filtro.getEncUnidade().toUpperCase() + "%"),
                    cb.like(cb.upper(joinUnidadeRecebeu.get(TbUnidade_.sgUnidade)), "%"
                            + filtro.getEncUnidade().toUpperCase() + "%")));
        ...
        if (ValidacaoHelper.isNotEmpty(filtro.getEncIdOperador())) {
            restricoesLista.add(joinTramite.get(TbTramite_.idUsuarioReceptor).get(TbUsuario_.idUsuario)
                    .in(filtro.getEncIdOperador()));
        ...
        if (filtro.getEncDataEnvioDe() != null && filtro.getEncDataEnvioAte() != null) {
            restricoesLista.add(cb.between(joinEncaminhamento.get(TbEncaminhamento_.dtCriacaoEncaminhamento),
                    DataHelper.getDataMin(filtro.getEncDataEnvioDe()),
                    DataHelper.getDataMax(filtro.getEncDataEnvioAte())));
        }
        if (filtro.getEncDataRespostaDe() != null && filtro.getEncDataRespostaAte() != null) {
            restricoesLista.add(cb.between(joinEncaminhamento.get(TbEncaminhamento_.dtRespostaTramite),
                    DataHelper.getDataMin(filtro.getEncDataRespostaDe()),
                    DataHelper.getDataMax(filtro.getEncDataRespostaAte())));
        }
    ...
    public List<TbManifestacao> getManifestacoesPorTramite(Date dataInicial, Date dataFinal, Integer idUnidade,
            Integer idUsuario, StatusEncaminhamentoEnum status) {
    ...
    public List<TbManifestacao> getManifestacoesSolucionadasSemEncaminhamento(Integer idUsuario) {
        try {
            StringBuilder select = new StringBuilder().append("SELECT m FROM TbManifestacao m ")
    ...
    @SuppressWarnings("unchecked")
    public List<TbManifestacaoxAnexo> getManifestacaoAnexoPorManifestacao(Integer idManifestacao) {
        String select = "select ma from TbManifestacao m inner join m.tbManifestacaoxAnexoCollection ma where m.idManifestacao = :idManifestacao";
        List<TbManifestacaoxAnexo> list = null;
        try {
            list = ((List<TbManifestacaoxAnexo>) getEntityManager().createQuery(select)
                    .setParameter("idManifestacao", idManifestacao).getResultList());
        } catch (Exception e) {
            return new ArrayList<TbManifestacaoxAnexo>(0);
        }
        return list;
    }
    
    public Collection<TbManifestacao> getPorClassificacao(Integer idClassificacao) {
        Collection<TbManifestacao> retorno = null;
        try {
            HashMap<String, Integer> map = new HashMap<String, Integer>();
            map.put("idClassificacao", idClassificacao);
            retorno = selectList("SELECT m FROM TbClassificacao c INNER JOIN c.tbManifestacaoCollection m where c.idClassificacao = :idClassificacao", map);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return retorno;
    ...
    public Collection<TbManifestacao> getPorSubClassificacao(Integer idSubClassificacao) {
        Collection<TbManifestacao> retorno = null;
        try {
            HashMap<String, Integer> map = new HashMap<String, Integer>();
            map.put("idSubClassificacao", idSubClassificacao);
            retorno = selectList("SELECT m FROM TbSubClassificacao s INNER JOIN s.tbManifestacaoCollection m where s.idSubClassificacao = :idSubClassificacao", map);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return retorno;
